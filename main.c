...
void merge(int ** a, int * a_size, int total_size, FILE * fp)
{
	...
	{	
		...
		if(k < MAX_PRINT_RES) fprintf(fp, "%d ", min);
		curr[tmp_m]++;
	}
	printf("\n");
	free(curr);
...
void sort(int * a, int size_of_array, int id)
{
	...
	{
		while(a[i] < x) {i++; switch_coroutine();} 
		while(a[j] > x) {j--; switch_coroutine();}
		if(i >= j) break;
		swap(a + i, a + j); switch_coroutine();
	}
...
{	
	...
	merge(a, a_size, total_size, stdout);
	fclose(fp);
